...
class WikiParser(object):
	...
	def _init_inline_parse(self):
		...
		return (
			...
			| Rule(OBJECT, r'(?P<obj_indent>\t*)\{\{\{ \s*? (\S+:.*\n)( (?:^.*\n)*? )(?P=obj_indent)\s*\}\}\}\s*?', process=self.parse_object)
	...
	def parse_list(self, builder, text, indent=None):
		...
		for newLine in allNewLines:
			...
			if not contained:
				lines.append(text[startIndex:newLine.end()])
				startIndex = newLine.end()
		
		#for group in allObjects:
		#	lines.append(text[startIndex:group.end()])
